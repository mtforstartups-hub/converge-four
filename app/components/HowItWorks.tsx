    ...
    <div className="converge-container mx-auto w-full px-6 md:px-14 bg-surface py-16 md:py-24 neon-divider">
      ...
      >
        {cardsData.map((card, index) => {
          ...
          return (
            <FadeIn key={card.id} delay={400 + index * 100} className="h-full">
              <div
                role="button"
                tabIndex={0}
                onClick={() => setActiveCard(isActive ? null : card.id)}
                className={`group relative overflow-hidden border border-neutral-300 hover:border-neutral-500 p-5 md:p-6 flex flex-col cursor-pointer text-center transition-all duration-300 outline-none h-full ${isActive ? "is-active border-neutral-500" : ""}`}
              >
